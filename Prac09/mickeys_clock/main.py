import pygame
import os
from clock import prepare_hand, get_angles #пути к файлам

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
IMAGES_DIR = os.path.join(BASE_DIR, 'images')

pygame.init()
screen = pygame.display.set_mode((795, 800)) #создаём окно
pygame.display.set_caption("Hello Kitty Clock ")
CENTER = (400, 400)#центр часов (точка вращения)

bg_path = os.path.join(IMAGES_DIR, 'mainclock.png')#фон часов
bg = pygame.image.load(bg_path).convert_alpha()#загружаем стрелки
...
while running: #игра работает постоянно
    for event in pygame.event.get():#проверяем события (закрытие окна)
        if event.type == pygame.QUIT:
            running = False

    h_ang, m_ang, s_ang = get_angles()#получаем углы стрелок

    screen.fill((255, 255, 255))
    screen.blit(bg, (0, 0))

    rot_m = pygame.transform.rotate(m_hand, m_ang)#поворачиваем стрелку
    rect_m = rot_m.get_rect(center=CENTER)#центрируем вращение
    screen.blit(rot_m, rect_m)#рисуем
